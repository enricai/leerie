...
- `paused_at`, `pushed_at`, and `killed_at` are mutually exclusive (a run cannot be in more than one terminal-or-paused state). If `paused_at` is set, `fly_machine_id` must also be set (you cannot pause a run without knowing where to resume it). If `killed_at` is set on a run that shows evidence of being a Fly/EC2 run (`fly_machine_id`, `ec2_instance_id`, `volume_id`, or `image_tag` non-null), at least one of `fly_machine_id`/`ec2_instance_id` must still be set (you cannot have destroyed a machine you don't have a pointer to). A run with none of that remote evidence ever set is a local (`nerdctl`) kill — there is no machine id to point to, so the invariant is exempt (N7: this previously rejected every local `leerie kill`, since a local run never has `fly_machine_id`).
