...
| `wiring_gate` | dict | audit record from `phase_wiring_gate` (DESIGN §5 *A wiring re-check on the fully-merged plan*, §8) — the final `wiring_judge` output `{plan_reviewed, wiring_defects, rationale}`. Detect-and-die, single pass (no `make_feedback_prompt` — see §5½ *Mechanical-feedback loops*): written once the judge's first round clears; a found defect `die()`s immediately instead of re-reconciling. Absent when the judge crashed every round. The deterministic `check_plan_wiring` that runs alongside it does not persist — it `die()`s or passes silently. |
