...
| per-worker cgroup memory cap (`worker_memory_max_bytes`) | auto-derived via `_auto_worker_memory_max` from the shared `leerie.slice/memory.max` budget and the live sibling `leerie-w-*` worker count (broker `slice` verb; `_cgroup_slice_info` / `_slice_worker_memory_max` — PENDING_ISSUES.md N9), NOT `/proc/meminfo`: `slice_max // (live_siblings + max_parallel + 1)`, floored at 256 MiB. Falls back to the legacy `/proc/meminfo`-derived basis (`_auto_worker_memory_max_legacy`, VM RAM split across `max_parallel + 1` slots, floored at 8 GiB) only when no broker/slice budget is readable (containment off, `--dangerously-allow-uncapped`) — there the shared-slice enforcement the new basis depends on is absent anyway. At the start of each wave (`phase_execute`, before constructing that wave's `asyncio.Semaphore`), `_degrade_max_parallel_for_wave` (M9+M3 DECISION 2026-08-09) computes, once and without blocking, the largest concurrency the wave can run at without driving the computed per-worker share below the measured ~6.3 GiB build+resident-claude peak, gated at an 8 GiB floor (`_WORKER_BUILD_PEAK_BYTES`, matching `_auto_worker_memory_max_legacy`'s own floor for the same peak) — shrinking that wave's semaphore size and logging a warning naming the degraded value when it doesn't fit, rather than admitting a worker the kernel is likely to OOM-kill mid-build. This replaces the retired `_await_worker_memory_admission`, which blocked each individual spawn for up to 10 minutes (polling every 5s) waiting on a per-worker share that could never rise while a live sibling run held its slot, then admitted anyway past the wait cap — measured 43x/18x slowdowns and 100% stall rates at higher `--max-parallel`. Overridable via `--worker-memory-max SIZE` / `LEERIE_WORKER_MEMORY_MAX` / `worker_memory_max` in `leerie.toml` (bypasses both the derivation and the admission gate). Suffixes K/M/G/T accepted | the kernel OOM-kills inside the worker's cgroup; sibling workers, the orchestrator, and host-side services (sshd, lima-guestagent) are not eligible victims. Enforcement goes through the **cgroup broker** (`scripts/cgroup-broker.py`), which the dropped-privilege orchestrator drives over a Unix socket — worker enrollment and limit-setting cannot be done from the orchestrator's own identity (a subtree merely `chown`ed after creation keeps its controller limit files root-owned; cross-scope migration needs common-ancestor write; both reproduced). The broker creates `<V2_ROOT>/leerie.slice/leerie-w-<sid>` (cgroup **v2** — `V2_ROOT` is the literal `/sys/fs/cgroup` rootful/Fly, or the systemd-delegated user slice under rootless containerd via `LEERIE_CGROUP_V2_ROOT`, DESIGN §6 *Rootless exception*) or the split `pids/`+`memory/` hierarchies at the fixed `V1_ROOT` (cgroup **v1/hybrid**, e.g. some Fly VMs, never rootless) and sets its `memory.max`. Local nerdctl needs the launcher's cgroup bind-mount — `bind-propagation=rshared` rootful, a plain bind (no propagation flag) rootless — + `--cgroupns=host` (default `--cgroupns=private` + `nsdelegate` blocks migration even for the broker); Fly's microVM exposes cgroupfs directly. `_cgroup_probe` asks the broker to round-trip a create+enroll+destroy — the true test of the worker path — and `_enforce_and_record_cgroup_containment` `die()`s before the first worker if it fails (unless `--dangerously-allow-uncapped`). See DESIGN §6 *Memory containment*. |
