...
unless the classifier's own last `likely_already_satisfied`/`_evidence`
signal is set, in which case exhaustion routes to `_finish_no_work_run`
...
| `likely_already_satisfied` | bool | classifier's additive signal that the task's deliverable already appears present on HEAD (DESIGN §8). Written on every `phase_classify` invocation, default `False`. Consulted by `phase_classification_gate` on retry-loop exhaustion to route to `_finish_no_work_run` instead of `die()` |
...
| `wiring_gate` | dict | audit record from `phase_wiring_gate` (DESIGN §5 *A wiring re-check on the fully-merged plan*, §8) — the final `wiring_judge` output `{plan_reviewed, wiring_defects, rationale}`. Written once the gate clears (immediately, or after re-reconciling). Absent when the judge crashed every round. The deterministic `check_plan_wiring` that runs alongside it does not persist — it `die()`s or passes silently. |
| `provision_recipe_gate` | dict | audit record from `phase_provision_gate` (DESIGN §8, §6½) — the final `provision_judge` output `{recipe_reviewed, recipe_failures, rationale}`. Written once the gate clears (immediately, or after re-provisioning). Absent when no recipe was detected (`kind: none`) or the judge crashed every round. |
...
- **classifier** — required: `categories` (array), `confidence`
  ...
  persists both to `st.data["likely_already_satisfied"]` /
  `st.data["likely_already_satisfied_evidence"]` on every invocation
  (default `False` / `""`). `phase_classification_gate` consults these on
