...
| `skip_coverage_check` | bool | whether the phase 2⅞½ task-coverage gate (the deterministic `check_required_items_coverage` floor + the `task_coverage_judge` worker) is suppressed. Resolved from `--skip-coverage-check` / `LEERIE_SKIP_COVERAGE_CHECK` / `skip_coverage_check` in `leerie.toml` / default `False`. When True, a plan that omits a required item is not caught before `phase_execute` spends. Added after run 488c42e5, where the judge counted a task item the task itself deferred as `missing_work` — unsatisfiable by any planner, with no operator override, unlike every sibling gate. Re-resolved fresh on every run, including `--resume` |
