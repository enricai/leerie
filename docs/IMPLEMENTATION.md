...
| `satisfied_probe` | `prompts/satisfied_probe.md` | sonnet | 0 or 1 per subtask (phase 3, before scheduling; skipped when `--skip-satisfied-check`) | `{satisfied: bool, evidence: str, unsatisfied_reason?: "artifact_missing"\|"behavior_gap"\|"partially_met"\|"cannot_verify", equivalent_coverage_exists?: bool}` — soft-drops subtasks already met on the base tree; a `satisfied: false` verdict with `unsatisfied_reason: "artifact_missing"` **and** `equivalent_coverage_exists: true` soft-drops the same way (`reason: "equivalent_coverage"`); any other combination, or either field absent, keeps the subtask. DESIGN §8 *A "not satisfied" verdict carries a typed reason* |
