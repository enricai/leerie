...
`dep_capture`, `fit_judge`, `splitter`, and `adherence_judge` are **absent**
from `MODEL_DEFAULT_PER_WORKER` — their `opus` defaults come from the
global `MODEL_DEFAULT` fallback.
