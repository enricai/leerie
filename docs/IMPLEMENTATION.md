...
│   │   └── worktree_bytes.py    maintainer tool: measures the marginal on-disk cost of one more worktree of a dependency tree (inode-deduped st_blocks, split by st_nlink), backing DESIGN §6's EXDEV/hardlink figures (writes tests/fixtures/worktree_bytes/summary.json; feeds no gate — see "Disk headroom (N30)"; outside pytest's testpaths)
...
What *has* been landed, per the work order's "land the measurement rather
than guessing" rule, is the static half:
`scripts/measure/worktree_bytes.py` walks a dependency tree and reports the
inode-deduped total split by `st_nlink`, committed as
`tests/fixtures/worktree_bytes/summary.json` and pinned against DESIGN's
quoted figures by `tests/test_worktree_bytes_measurement.py`. It measures
the two regimes the ~20x claim rests on and **feeds no gate** — it exists so
the explanation is reproducible. It is deliberately *not* the `df` delta
above: it can show what a shared store hardlinks away, but not what a real
second checkout costs in a real container, which is the number a rebuilt
bound would need.
...
4. **`_invoke`'s prompt staging** is the second reactive one, and closes the
   ...
   write leerie makes per worker — and its existing reap-then-reraise guard
   now converts `errno.ENOSPC` to `DiskLowSpace` on the same terms as (3).
