...
Verbs `--kill` and `--stop` accept an optional `--runtime
<local|fly|ec2>` flag — validated by the launcher (bash) against
`local`/`fly`/`ec2`, matching the top-level `RUNTIME_VALUES` enum
(`local|fly|ec2` — see "Remote execution mode" below) that gates the
`--runtime` flag for launching a new run. `--finalize` remains
narrower: it still validates only `local`/`fly` (rejecting `ec2` with
an error) since `--finalize --runtime ec2` has not shipped.
