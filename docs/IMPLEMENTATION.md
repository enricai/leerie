...
Host-side rc=75 branch (`leerie:2794-2834`) sets `container_rc=130`
...
| `/health` | GET | launcher (chain-submit startup wait) | Liveness probe (always 200). |
...
when the run-directory lock is held. The launcher's rc=75 branch then
...
| `image_tag` | str \| null | Full Fly registry image tag (e.g. `registry.fly.io/leerie:0.6.7`) recorded at provision time. Used by `resume_machine()` to detect version drift: if the current `$FLY_IMAGE_TAG` differs from the stored value, the machine's image is updated via `flyctl machine update --image --skip-start` before starting. Updated in place on successful image update. Null for local-runtime runs or legacy Fly runs provisioned before the field was introduced (in which case the image-update check is skipped — no false positive possible). |
