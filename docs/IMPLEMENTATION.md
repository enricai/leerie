...
| integrator **crash** (`_run_checked_loop` returns `None`) | infrastructure failure, not a verdict — `rescue_integrator_work()` captures the in-progress resolution to `refs/leerie/rescue/<run-id>/<sid>` **before** the merge is aborted, `blocked[sid]` is recorded, and the die message names the ref plus its `cherry-pick --no-commit` recovery command. `--resume` retries the integration |

`rescue_integrator_work(staging, sid, run_id) -> str | None` returns the rescue
...
|--------|---------------|-------------|----------------|
| Classifier | `check_classifier_output(result, repo_root, judge_confirmed=frozenset())` | `CATEGORY_NO_DIR`, `EMPTY_WHY`, `EMPTY_EVIDENCE`, `MANY_CATEGORIES`, `SAME_WORK_RISK`, `TEST_OWNERSHIP_RISK`, `LOW_CONFIDENCE` | `judgment_check_rounds` (3) |
...
test rejects the primary case and, because `prompts/plan_overlap_judge.md`
ties the confidence floor to artifact verifiability, also drives the judge
to self-downgrade below the `LOW_CONFIDENCE` threshold for findings that
were correct. Only a path present in neither the tree nor the plan is
flagged. Path comparison is normalized (`_normalize_artifact_path`:
...
`artifact` is a **logical** artifact name, not necessarily a path —
`prompts/plan_overlap_judge.md` asks for "the colliding exported artifact,
e.g. `AuthShell component`", and on a docs- or config-heavy plan the natural
name embeds a path inside prose (`docs/USAGE.md bare-verb rewrite`). So
`PHANTOM_ARTIFACT` **tokenizes** the artifact on whitespace, keeps the
path-shaped tokens (containing `/`, or ending in a `_CODE_EXTS` extension),
and flags only when *no* path-shaped token resolves against the plan or the
tree. An artifact with no path-shaped token at all is a pure logical name
and is not path-checkable, so it is never flagged. Treating the whole string
as one path (the pre-fix behavior) false-positived every descriptive
artifact name: run `e2882da6…` produced 6 spurious `PHANTOM_ARTIFACT`
issues on an otherwise-valid emission, and the retry those issues forced is
what produced the fatal duplicate-pair emission below.
...
artifact — `artifact` is a single string, so per-artifact rows are the only
encoding available to the judge — and `_apply_overlap_collisions` already
absorbs the repeat via its `skipped_redundant` branch. What matters is not
the `resolution` *string* but the resolved **effect**: the dropped sid for a
`drop_*`, or the sorted endpoint pair for a `merge`. Two rows whose effects
are identical are coalesced by `_validate_overlap_judge_output` into one
collision (artifacts joined, both `merge_feasibility` statements preserved)
and applied. Two rows whose effects *differ* — e.g. the same pair emitted
...
`filter_satisfied_subtasks`) with an all-empty mapping to prune dropped ids. This
...
ever provided is **left intact** so `validate_plan` still surfaces it as a genuine
...
*Cross-domain capability tags*) and `validate_plan` checks provider-existence globally,
...
deterministic `_normalize_pip_installs` / `validate_provision_recipe` miss.
...
(bounded by `judgment_check_rounds`, cut short earlier by that loop's own
oscillation guard on a repeated issue signature), `die()`ing on exhaustion.
The judge's payload is "the reconciled subtask set" on every round, not
just the first: the re-drive's feedback callback re-invokes `phase_reconcile`
on the freshly re-planned output before the next round's judge call, since
a re-plan (one planner per category, no cross-category tag visibility) can
reintroduce cross-domain `provides`/`requires` drift the first reconcile
pass already resolved. Persists to `state.data["coverage_gate"]`.
...
`DROP_BREAKS_GRAPH`, `DUPLICATE_PAIR` (already computed in that same
function), and
...
| `coverage_gate` | dict | audit record from the phase 2⅞½ task-coverage gate (`phase_planning_coverage_gate`, DESIGN §8 *Independent adversarial verification*) — the final `task_coverage_judge` output `{task_covered, coverage_gaps, rationale}`. Written once the gate clears (immediately, or after re-planning). Absent when the judge crashed every round (degrade path returns without persisting this key). Replaces the planner's self-graded `task_understanding` confidence axis, which no longer gates in `check_planner_output`. |
...
- **classifier** — required: `categories` (array), `confidence`
  ...
  `_finish_no_work_run` (the same terminal state `detect_no_work`
