...
| 2 | sum `st_blocks`, de-duplicated by inode | measured **total**, not marginal: 1.35 GiB where the true cost of one more tree is 102 MiB, a ~13x over-demand on any host whose package-manager store shares a mount |
...
the two regimes the ~13x claim rests on and **feeds no gate** — it exists so
