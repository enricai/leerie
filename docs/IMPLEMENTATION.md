...
DESIGN §12 *Opus-judgment, sonnet-workhorse (historical)* for why a
...
| no_work_judge | sonnet | independent adversarial verifier of the classifier's `likely_already_satisfied` claim on the converged-gate path (DESIGN §8 *The healthy-path consumer*); wired into `phase_classification_gate` via `_confirm_no_work_on_converged_gate` |
...
`artifact_registry`, `task_coverage_judge`, `integration_judge`,
`no_work_judge`, and
...
Twenty worker types (`WORKER_TYPES`, plus the global override), each
...
integration_judge, no_work_judge, artifact_registry, rebaser. Global
override: `LEERIE_MODEL` / `--model` / `model`.
...
| `--max-rate-limit-wait SEC` | `21600` (6 h) | Longest rate-limit reset `main()` will sleep through before it stops auto-resuming and takes the `out_of_credits` disposition instead (worktree-only cleanup, resume hint, `EXIT_LOCKED`). The five-hour session window is bounded and worth waiting out; the weekly window resets at a fixed per-account instant that can be days away, and sleeping on it holds the run-directory flock and blocks every other run on the repo. Keyed on the **computed wait**, never on `rateLimitType`, so a renamed limit type cannot silently disable it. Also `LEERIE_MAX_RATE_LIMIT_WAIT` or `max_rate_limit_wait_sec` in `leerie.toml`. |
...
| `--skip-classification-check` | off | Skip `phase_classification_gate` (DESIGN §8) entirely: `classification_judge` never spawns and the classifier's own category set stands. The escape hatch for a gate that will not converge — it runs before any provision/plan spend and `die()`s on exhaustion, so a stuck gate otherwise takes the whole run. A miscategorized set produces the wrong planners, so prefer refining the task description first. Also `LEERIE_SKIP_CLASSIFICATION_CHECK` or `skip_classification_check` in `leerie.toml`. |
...
| `LEERIE_MODEL_<WORKER>` | `model_<worker>` | Per-worker override (e.g. `LEERIE_MODEL_IMPLEMENTER=opus`). Overridden by `--model-<worker>`. `<worker>` ∈ every `WORKER_TYPES` entry: `classifier`, `planner`, `reconciler`, `plan_overlap_judge`, `satisfied_probe`, `provision`, `implementer`, `integrator`, `conformer`, `fit_judge`, `splitter`, `adherence_judge`, `classification_judge`, `wiring_judge`, `provision_judge`, `task_coverage_judge`, `artifact_registry`, `integration_judge`, `no_work_judge`, `rebaser`. Unset → every worker → `sonnet`. |
...
| `LEERIE_MAX_RATE_LIMIT_WAIT` | `max_rate_limit_wait_sec` | Longest rate-limit reset to sleep through, in seconds. Overridden by `--max-rate-limit-wait`. Unset → default `21600` (6 h). Beyond it, the run pauses resumably rather than sleeping — see the `--max-rate-limit-wait` row above. |
...
| `LEERIE_SKIP_CLASSIFICATION_CHECK` | `skip_classification_check` | Skip the `classification_judge` category-set gate entirely (truthy → skip). Overridden by `--skip-classification-check`. Unset → default `false`. |
...
| `classification_judge` | `prompts/classification_judge.md` | sonnet | 0 or 1 (phase 1½, adversarial re-check of the classifier's category set) | `{categories_reviewed[], miscategorizations[], rationale}` — independent verification the classifier's category set covers the task. DESIGN §8 *Independent adversarial verification* |
| `no_work_judge` | `prompts/no_work_judge.md` | sonnet | 0 or 1 (phase 1½, only when the gate converges with `likely_already_satisfied` + evidence; suppressed by `--skip-satisfied-check`) | `{confirmed: bool, evidence: str, checked?[]}` — independently re-verifies the classifier's already-satisfied claim against the current checkout (SATISFIED_PROBE_TOOLS scope); only `confirmed: true` with non-empty evidence routes to `_finish_no_work_run`, everything else falls through to planning. DESIGN §8 *The healthy-path consumer* |
...
| `satisfied_probe` | `prompts/satisfied_probe.md` | sonnet | 0 or 1 per subtask (phase 3, before scheduling; skipped when `--skip-satisfied-check`) | `{satisfied: bool, evidence: str, unsatisfied_reason?: "artifact_missing"\|"behavior_gap"\|"partially_met"\|"cannot_verify", equivalent_coverage_exists?: bool, sibling_invalidation_risk?: bool}` — soft-drops subtasks already met on the base tree; a `satisfied: false` verdict with `unsatisfied_reason: "artifact_missing"`, `equivalent_coverage_exists: true`, **and an explicit `sibling_invalidation_risk: false`** soft-drops the same way (`reason: "equivalent_coverage"`); any other combination, or any field absent, keeps the subtask — the sibling channel stays keep-only by construction. DESIGN §8 *A "not satisfied" verdict carries a typed reason* |
...
| `--append-system-prompt` | injects the worker's role prompt — read from `prompts/*.md` for every `WORKER_TYPES` worker (classifier/planner/reconciler/plan_overlap_judge/satisfied_probe/provision/implementer/integrator/conformer/fit_judge/splitter/adherence_judge/classification_judge/wiring_judge/provision_judge/task_coverage_judge/artifact_registry/integration_judge/no_work_judge/rebaser), plus the post-run / finalize workers pr_writer, judge, dep_capture, and patch_generator |
...
all-required pass, which collapses the combinatorial explosion. The
all-required pass carries one behavioral disclosure: `satisfied_probe`'s
typed not-satisfied fields (`unsatisfied_reason`,
`equivalent_coverage_exists`, `sibling_invalidation_risk`) default to
...
|   • Classification gate | `phase_classification_gate` | independent adversarial verifier of the classifier's category set (DESIGN §8 *Independent adversarial verification*). One `classification_judge` worker attacks the chosen categories against the task + codebase; a non-empty `miscategorizations` array (a missing category the work requires, or a spurious one) re-drives `phase_classify` via `_run_checked_loop` (bounded by `judgment_check_rounds`, and cut short earlier by `_run_checked_loop`'s own oscillation guard when a round's issues repeat an earlier round's exactly — DESIGN §8 *The CRITIC retry pattern's oscillation guard*). Skipped entirely by `--skip-classification-check`, which returns `False` without spawning the judge and without writing the audit key — a bypass is not a clean-pass judgment, matching every sibling skip path. Across rounds, the gate accumulates a `judge_confirmed` set — every category the judge has reviewed without objection or explicitly asked to add — and passes it into the re-invoked `phase_classify`, so `check_classifier_output`'s `SAME_WORK_RISK`/`TEST_OWNERSHIP_RISK`/`CATEGORY_NO_DIR` self-checks never strip a category the judge has already vetted. `CATEGORY_NO_DIR` honours it for the same reason as the two RISK advisories, but on a **single** category rather than a pair, and for `infrastructure` as well as `documentation`. Separately, what counts as evidence for a category is `CATEGORY_EVIDENCE` + `_category_evidence_found`: directories matched case-insensitively at the root **or one level down** (`src/infrastructure/`), root marker files by case-folded stem (`cdk.json`, `readme`), and root globs (`*.tf`). Suppression is empty-handed on the initial classify, so a signal that mis-describes a correct classification fires before any judge runs — measured at 27 firings across 19 runs, all false. `_category_evidence_summary` derives the advisory text from the same table so the remedy it names cannot drift from the rule. The set is monotone except under an evidenced retraction (DESIGN §8 *`judge_confirmed` is monotone except under an evidenced retraction*): a narrower later round does not retract an earlier confirmation, but an **evidenced** `spurious_category` does — `_check` applies `judge_confirmed.difference_update(spurious)` and will not re-add a category the same round called spurious. `_gating_spurious_categories` is the single definition of "concretely objected to", shared by `_check` and the feedback builder; a vague entry (empty `concrete_work_evidence`) neither gates nor retracts. `_on_feedback` names the drop list outright in the re-classify prompt and `die()`s rather than emit a keep-list and drop-list that intersect — the failure mode of the contradiction is not a wrong answer but an unsatisfiable instruction that silently burns the round budget. On exhaustion, `die()`s with the residuals named and the **actual** number of re-classify rounds that ran (tracked by the gate, not read off the cap: `_run_checked_loop` fires feedback only while `rnd < max_rounds - 1`, so a 3-round budget re-classifies at most twice, and once when the oscillation guard aborts early) — UNLESS `st.data["likely_already_satisfied"]` is `True` with non-empty evidence, in which case it routes to `_finish_no_work_run` (the same terminal state `_detect_no_work` produces post-plan; DESIGN §8 *Reaching the cleared-but-empty state from classification*) and returns `True`, signaling the caller (`_run_phases`) to stop the pipeline. Gates before provision/plan spend. Runs inside the `plans_after_classify` checkpoint block (DESIGN §6 "Resumable planning"), so a resume past classify skips it. Persists to `state.data["classification_coverage_gate"]`. **On the CONVERGED path** (gate clean), `_confirm_no_work_on_converged_gate` consults the same `likely_already_satisfied` signal — but through an independent `no_work_judge` (read-only, `SATISFIED_PROBE_TOOLS`, current-checkout-only) rather than raw trust: only `confirmed: true` with non-empty evidence routes to `_finish_no_work_run` (audit record in `state.data["no_work_confirmation"]`, returns `True`); a dispute, crash, timeout, or empty evidence falls through to planning unchanged, and `skip_satisfied_check` suppresses the consult entirely (DESIGN §8 *The healthy-path consumer*). |
...
| Check | Catches |
|-------|---------|
| **deterministic duplicate-provider floor** (`check_duplicate_providers(plans) -> list[str]`, `DUPLICATE_PROVIDER` / `DUPLICATE_PROVIDER_NO_OVERLAP`) — runs **before** the cheap-skip and independently of `--skip-overlap-judge`, so it fires on every path including single-planner runs and a `plan_overlap_judge` `WorkerError` | Two tiers over the same tag→providers map (`_iter_duplicate_provider_pairs`, which yields `(sid_a, sid_b, tag, shared_files, kind)` where `kind ∈ {"overlap", "no_overlap"}`). **`DUPLICATE_PROVIDER`** — same **`provides` tag** *and* intersecting `files_likely_touched`: duplicate work on one file; routed into the M11 auto-merge below. **`DUPLICATE_PROVIDER_NO_OVERLAP`** — same tag, no shared file: advisory only, never auto-merged (folding is right for duplicate work and wrong for a mis-declared tag, and the synthesizer cannot tell them apart). Pure set logic (DESIGN *Language-to-JSON*). Path comparison is `_paths_designate_same_file` (component-wise, absorbing a repo-root prefix on either side), not raw string equality. **Three load-bearing exclusions:** (1) pairs sharing a non-`None` `_cofile_cluster` (§5½ (P1) *Sub-file*); (2) pairs sharing a non-`None` `_newfile_owner_id` — multi-file splitter chunks, which inherit the parent's `provides` verbatim and are not covered by (1) since only `_cofile_child` sets a cluster; (3) **no-overlap tier only** — pairs ordered by transitive `depends_on`/`requires` (`_transitive_predecessors` over `_build_predecessor_graph`), the legitimate shared-umbrella-tag chain. Exclusion (3) deliberately does **not** apply to the overlap tier (DESIGN §5). |
...
(300s) and re-exec the same way. **Ceiling:** if the computed
`wait_seconds` exceeds the resolved `max_rate_limit_wait_sec` cap (default
6h; `--max-rate-limit-wait` > `LEERIE_MAX_RATE_LIMIT_WAIT` >
`max_rate_limit_wait_sec` in leerie.toml > `DEFAULT_CAPS`, via
`resolve_max_rate_limit_wait_sec`), neither arm sleeps —
`main()` takes the `out_of_credits` disposition instead (worktree-only
cleanup, best-effort `dep_capture`, resume hint, `EXIT_LOCKED`), because a
weekly-limit reset can be days out and sleeping holds the run-directory
flock. Keyed on the computed wait, never on `RateLimitedExit.limit_type`
(carried from the `rate_limit_event`'s `rateLimitType` and used only in the
pause message), so a renamed limit type cannot silently disable it. Both clock-based arms route through
...
An issue in any of these tables **gates** — costing a round and re-driving the worker — unless its label is listed in `_ADVISORY_ISSUE_LABELS`, which is the authority; `_partition_issues_by_severity` splits on it. Read that set rather than a per-row annotation here, which would be a copy of a derived fact. Worth knowing for `CATEGORY_NO_DIR` in particular: it gates, while the two RISK advisories beside it in the `judge_confirmed` suppression do not.
...
same normalizer, plus `_paths_designate_same_file` so a repo-root prefix on
one side does not read as disjoint. **`NO_FILE_OVERLAP` is not emitted when
the two subtasks share a `provides` tag** — the tag is its own grounding,
and for a file neither subtask has created yet each planner invents the
path, so requiring intersection suppressed correct findings (DESIGN §5 *A
deterministic floor underneath the judge*). `DROP_BREAKS_GRAPH` likewise
subtracts tags a **surviving** subtask still provides before flagging an
orphan.
...
| rate-limit wait ceiling (`max_rate_limit_wait_sec`) | 21600 s (6 h), or `--max-rate-limit-wait SEC` / `LEERIE_MAX_RATE_LIMIT_WAIT` / `max_rate_limit_wait_sec` in `leerie.toml` (positive integer; `resolve_max_rate_limit_wait_sec`, same `_resolve_positive_int_pref` ladder as `resolve_worker_timeout_sec`) | Longest rate-limit reset `main()`'s `except RateLimitedExit` arm will sleep through before it stops auto-resuming and takes the `out_of_credits` disposition instead (worktree-only cleanup, resume hint, `EXIT_LOCKED`). Sized to the subscription's windows: the five-hour session limit is bounded and worth waiting out; the weekly limit resets at a fixed per-account instant that can be days away, and sleeping on it holds the run-directory flock. Keyed on the **computed wait**, never on `rateLimitType`, so a renamed limit type cannot disable it. See DESIGN §6 *A ceiling on the wait*. |
...
(bounded by `judgment_check_rounds`; an evidenced `spurious_category` retracts
the category from the gate's `judge_confirmed` keep-list), `die()`ing on
exhaustion — unless `state.data`'s OR-accumulated
`likely_already_satisfied`/`_evidence` signal
...
| `plans_after_overlap_judge` | list[dict] | the per-phase planning checkpoint for `phase_overlap_judge` (post-collision-resolution `plans`, DESIGN §5 *Cross-domain surface overlap*). Same absence/presence and resume-cursor semantics as `plans_after_classify`. |
...
| `skip_classification_check` | bool | whether `phase_classification_gate` (DESIGN §8 *Independent adversarial verification*) is suppressed. Resolved from `--skip-classification-check` / `LEERIE_SKIP_CLASSIFICATION_CHECK` / `leerie.toml` / default `False`. |
...
| `under_scope_warnings` | dict[str, dict] | `{touched, declared, reason}` per subtask that committed work but touched **none** of its `files_likely_touched`, from `check_diff_scope` (non-fatal signal log; DESIGN §9 *Scope is checked in both directions*). Comparison via `_declared_path_covers` — component-wise, absorbs a repo-root prefix on either side, and a directory-shaped declaration covers files beneath it. |
...
| `plan_overlap_judge` | `prompts/plan_overlap_judge.md` | cross-domain surface-overlap analysis, phase 2¾ (DESIGN §5) |
| `fit_judge`      | `prompts/fit_judge.md` | per-subtask decomposition fit verdict (DESIGN §5½) |
| `splitter`       | `prompts/splitter.md` | coupled-minority subtask splitter (DESIGN §5½) |
| `adherence_judge`| `prompts/adherence_judge.md` | whole-plan instruction-adherence score, phase 2⅞ (DESIGN §12 sibling) |
| `satisfied_probe`| `prompts/satisfied_probe.md` | per-subtask base-tree already-satisfied probe, phase 3 (DESIGN §8) |
| `no_work_judge`  | `prompts/no_work_judge.md` | independent verifier of the classifier's already-satisfied claim on the converged gate path, phase 1½ (DESIGN §8 *The healthy-path consumer*) |
...
| `integration_judge` | `prompts/integration_judge.md` | post-merge behavioral-correctness verifier inside `integrate_wave` (DESIGN §8) |
| `rebaser`        | `prompts/rebaser.md` | finalize-time rebase-onto-base worker (DESIGN §6 *Finalization*) |
| `pr_writer`      | `prompts/pr_writer.md` | invoked by `phase_finalize` when `push_will_happen` is true |
| `dep_capture`    | `prompts/dep_capture.md` | post-run dependency-capture worker (DESIGN §6½) |
