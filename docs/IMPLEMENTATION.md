...
| `--strict-mcp-config` | unconditional for every worker, independent of and prior to any `mcp__*` denylist enumeration — cuts MCP tool exposure off at the source regardless of what `.claude.json` seeding copies into the container (measured: 4 `mcp_servers` / 46 MCP tools baseline → 0/0, rc 0, CLI 2.1.234). `claude_p` never passes `--mcp-config`, so this flag can't strand a worker with server config it can't use |
