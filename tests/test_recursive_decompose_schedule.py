"""Integration test: _recursive_decompose leaves feed _schedule() into waves.
...
existing Phase 3 scheduler (DESIGN §5½ (P1) end-of-pipeline claim):
  ...
  _recursive_decompose() → flat leaf set → _schedule(plans) → topo-sorted waves

Tests verify:
  - A stubbed low-fit migration subtask decomposes into leaf subtasks
    with valid ids/provides/depends_on that _schedule() and _validate_plan()
    accept without errors.
  - Independent leaves land in wave 0; a dependent leaf lands in wave 1.
  - Leaf ids preserve a valid domain prefix so _schedule() cross-domain
    wiring works and _validate_plan()'s id-prefix check passes.
...
# ---------------------------------------------------------------------------
# Test 1: leaf ids carry valid domain prefix through to _schedule()
# ---------------------------------------------------------------------------

def test_leaf_ids_preserve_domain_prefix(leerie):
    """Leaves from _recursive_decompose carry a valid domain prefix so
    _schedule()'s cross-domain wiring and _validate_plan()'s id-prefix
...
# ---------------------------------------------------------------------------
# Test 2: leaf set feeds _schedule() and produces correct wave structure
# ---------------------------------------------------------------------------

def test_leaves_feed_schedule_correct_waves(leerie, capsys):
    """Leaves produced by _recursive_decompose (stubbed) pass through
    _schedule() and emerge in the correct topo-sorted waves.
    ...
    subtasks, waves = leerie._schedule([plan])
    ...
    # _schedule() must not emit a WARNING (no blocked domains).
    out = capsys.readouterr().out
    assert "WARNING" not in out


# ---------------------------------------------------------------------------
# Test 3: _validate_plan accepts the leaf set without errors
# ---------------------------------------------------------------------------

def test_leaves_survive_validate_plan(leerie):
    """_validate_plan() must accept the full leaf set from _recursive_decompose:
    ...
    # _validate_plan() calls die() on any error; no error means it passes.
    leerie._validate_plan(subtasks)


# ---------------------------------------------------------------------------
# Test 4: end-to-end seam — _recursive_decompose leaves → _schedule() → waves
# ---------------------------------------------------------------------------

def test_recursive_decompose_to_schedule_seam(leerie, capsys):
    """Full seam: run _recursive_decompose on an oversized migration subtask
    with stubbed claude_p, take the leaves, build a ready plan, call
    _schedule(), and assert the wave structure is valid.

    This is the '92% leaves → _schedule()' end-of-pipeline claim from the
    ...
    async def fake_claude_p(*args, schema_key, sid="", user_prompt="", **kwargs):
        ...
        if schema_key == "splitter":
            # Label-only mode: _partition_files owns the file assignment; the
    ...
    # Feed the leaves into _schedule() via a ready plan.
    plan = _ready_plan("dependency-migration", *leaves)
    subtasks, waves = leerie._schedule([plan])

    # _schedule() accepted all leaves.
    assert len(subtasks) == 3

    # Leaves from _partition_files are independent → all in wave 0.
    assert len(waves) == 1
    assert set(waves[0]) == {leaf["id"] for leaf in leaves}

    # _validate_plan accepts the leaves without errors.
    leerie._validate_plan(subtasks)
