...
`.tool-versions` / `mise.toml` go pin, leerie synthesizes a mise
...
def test_synthesizes_from_simple_gomod(leerie, tmp_path):
...
def test_synthesizes_with_patch_version(leerie, tmp_path):
...
def test_existing_go_version_file_blocks_synth(leerie, tmp_path):
...
def test_existing_tool_versions_with_go_blocks_synth(leerie, tmp_path):
...
def test_existing_tool_versions_without_go_does_not_block(leerie, tmp_path):
    """A `.tool-versions` that pins other tools but not Go is NOT a
    blocker — leerie synthesizes the Go pin so mise installs Go on top of
...
def test_existing_mise_toml_with_go_blocks_synth(leerie, tmp_path):
...
def test_existing_mise_toml_without_go_is_concatenated(leerie, tmp_path):
...
def test_polyglot_go_plus_nvmrc_injects_node_pin(leerie, tmp_path):
...
def test_dotted_mise_toml_with_go_blocks_synth(leerie, tmp_path):
...
def test_dotted_mise_toml_node_pin_survives_synth(leerie, tmp_path):
...
def test_capital_v_prefix_is_stripped(leerie, tmp_path):
...
def test_asdf_nodejs_alias_normalized_to_node(leerie, tmp_path):
...
def test_tool_versions_nodejs_alone_normalized_to_node(leerie, tmp_path):
...
def test_nvmrc_v_prefix_is_stripped(leerie, tmp_path):
...
def test_polyglot_go_plus_python_version(leerie, tmp_path):
...
def test_existing_mise_toml_pin_wins_over_idiomatic_file(leerie, tmp_path):
    ...
    idiomatic discovery), so leerie matches that precedence."""
    (tmp_path / "go.mod").write_text("go 1.22\n")
    (tmp_path / ".nvmrc").write_text("20.11.0\n")
    (tmp_path / "mise.toml").write_text('[tools]\nnode = "18.17.0"\n')
    run_dir = tmp_path / ".leerie" / "runs" / "x"
...
def test_tool_versions_multiple_entries_injected(leerie, tmp_path):
...
def test_malformed_gomod_returns_none(leerie, tmp_path):
...
def test_run_dir_is_created_if_missing(leerie, tmp_path):
