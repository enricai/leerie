...
def test_provision_schema_exists(leerie):
    ...
    assert "provision" in leerie.SCHEMAS
    schema = leerie.SCHEMAS["provision"]
    assert schema["type"] == "object"


def test_provision_requires_recipe(leerie):
...
def test_provision_command_is_argv_list(leerie):
...
def test_provision_in_worker_types(leerie):
    """`provision` must be wired into WORKER_TYPES so resolve_models()
    picks up its --model-provision / LEERIE_MODEL_PROVISION / model_provision
