...
def test_tty_path_stores_answer_under_question_id(leerie, state, monkeypatch):
...
def test_tty_path_empty_input_stores_empty_string(leerie, state, monkeypatch):
...
def test_non_tty_writes_pending_clarifications_json(leerie, state,
                                                    monkeypatch):
...
def test_non_tty_saves_state_before_exiting(leerie, state, monkeypatch):
...
def test_non_tty_exit_code_is_exit_needs_answers(leerie, state, monkeypatch):
    ...
    assert leerie.EXIT_NEEDS_ANSWERS == 10
