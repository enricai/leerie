...
def _impl_repo(tmp_path):
    ...
    run_git_repo_first(d, "add", "-A")
    run_git_repo_first(d, "commit", "-qm", "implementer")
    return d, _rev(d)
...
class TestClobberedOwnedFiles:
    def test_legit_edit_not_flagged(self, leerie, tmp_path):
        ...
        run_git_repo_first(d, "add", "-A")
        run_git_repo_first(d, "commit", "-qm", "conformer: fix a")
        r = asyncio.run(leerie._clobbered_owned_files(str(d), "run", impl))
        assert r == []

    def test_revert_to_base_flagged(self, leerie, tmp_path):
        d, impl = _impl_repo(tmp_path)
        run_git_repo_first(d, "checkout", "run", "--", "a.py", "b.py")  # the incident
        run_git_repo_first(d, "add", "-A")
        run_git_repo_first(d, "commit", "-qm", "conformer: revert")
        r = asyncio.run(leerie._clobbered_owned_files(str(d), "run", impl))
        assert set(r) == {"a.py (reverted-to-base)", "b.py (reverted-to-base)"}

    def test_deletion_flagged(self, leerie, tmp_path):
        d, impl = _impl_repo(tmp_path)
        run_git_repo_first(d, "rm", "-q", "b.py")
        run_git_repo_first(d, "commit", "-qm", "conformer: remove b")
        r = asyncio.run(leerie._clobbered_owned_files(str(d), "run", impl))
    ...
    def test_untouched_impl_file_never_flagged(self, leerie, tmp_path):
        ...
        run_git_repo_first(d, "add", "-A")
        run_git_repo_first(d, "commit", "-qm", "conformer: touch c")
        r = asyncio.run(leerie._clobbered_owned_files(str(d), "run", impl))
        assert r == []
    ...
    def test_new_file_addition_not_flagged(self, leerie, tmp_path):
        ...
        run_git_repo_first(d, "add", "-A")
        run_git_repo_first(d, "commit", "-qm", "conformer: fix a, add d")
        r = asyncio.run(leerie._clobbered_owned_files(str(d), "run", impl))
        assert r == []
    ...
    def test_round0_snapshot_catches_multi_round_clobber(self, leerie, tmp_path):
        ...
        run_git_repo_first(d, "checkout", "run", "--", "a.py")
        run_git_repo_first(d, "add", "-A")
        run_git_repo_first(d, "commit", "-qm", "conformer: round0 revert a")
        r0_head = _rev(d)
        # round 1: legit edit to b
        (d / "b.py").write_text("IMPL b + fix\n")
        run_git_repo_first(d, "add", "-A")
        run_git_repo_first(d, "commit", "-qm", "conformer: round1 fix b")
    ...
    def test_mode_only_change_with_identical_blob_is_skipped(self, leerie, tmp_path):
        ...
        (d / "c.py").chmod(0o755)
        run_git_repo_first(d, "add", "-A")
        run_git_repo_first(d, "commit", "-qm", "implementer: chmod +x c.py")
        impl2 = _rev(d)
...
class TestRollbackRestoresClobber:
    def test_rollback_to_impl_head_restores_clobbered_files(
        ...
        run_git_repo_first(d, "checkout", "run", "--", "a.py")
        run_git_repo_first(d, "add", "-A")
        run_git_repo_first(d, "commit", "-qm", "conformer: clobber a")
...
def _staging_repo(tmp_path):
    ...
    run_git_repo_first(d, "checkout", "-q", "-b", "run")
    (d / "a.py").write_text("IMPL a\n")
    (d / "b.py").write_text("IMPL b\n")
    run_git_repo_first(d, "add", "-A")
    run_git_repo_first(d, "commit", "-qm", "leerie: integrate feat-001")
...
class TestFinalPassBaseIsASnapshot:
    ...
    def test_fork_point_base_still_flags_a_genuine_revert(
        ...
        d, working, staging_before = _staging_repo(tmp_path)
        run_git_repo_first(d, "checkout", "main", "--", "a.py")  # the real data-loss shape
        run_git_repo_first(d, "add", "-A")
        run_git_repo_first(d, "commit", "-qm", "conformer: revert a")
        base = asyncio.run(
            leerie._merge_base_sha(str(d), working, staging_before))
        r = asyncio.run(
            leerie._clobbered_owned_files(str(d), base, staging_before))
    ...
    def test_fork_point_base_still_flags_a_deletion(self, leerie, tmp_path):
        d, working, staging_before = _staging_repo(tmp_path)
        run_git_repo_first(d, "rm", "-q", "b.py")
        run_git_repo_first(d, "commit", "-qm", "conformer: delete b")
        base = asyncio.run(
            leerie._merge_base_sha(str(d), working, staging_before))
        r = asyncio.run(
            leerie._clobbered_owned_files(str(d), base, staging_before))
...
class TestProtectedPathsSince:
    ...
    def test_only_protected_subset_of_diff_returned(self, leerie, tmp_path):
        ...
        run_git_repo_first(d, "add", "-A")
        run_git_repo_first(d, "commit", "-qm", "touch protected and ordinary")
        r = asyncio.run(leerie._protected_paths_since(str(d), base))
...
class TestUnprefixedConformerCommits:
    ...
    def test_all_commits_prefixed_returns_empty(self, leerie, tmp_path):
        d, impl = _impl_repo(tmp_path)
        run_git_repo_first(d, "commit", "--allow-empty", "-qm", "conformer: clean pass")
    ...
    def test_unprefixed_commit_subject_returned(self, leerie, tmp_path):
        d, impl = _impl_repo(tmp_path)
        run_git_repo_first(d, "commit", "--allow-empty", "-qm", "oops forgot the prefix")
        r = asyncio.run(leerie._unprefixed_conformer_commits(str(d), impl))
