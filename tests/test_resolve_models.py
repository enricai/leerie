...
def repo_root(tmp_path, monkeypatch):
    """An empty repo-root with every LEERIE_MODEL* env var unset."""
...
def test_per_worker_env_overrides_global_env(leerie, repo_root, monkeypatch):
    ...
    monkeypatch.setenv("LEERIE_MODEL_IMPLEMENTER", "opus")
    models = leerie.resolve_models(repo_root, ns())
...
def test_per_worker_cli_beats_everything(leerie, repo_root, monkeypatch):
    (repo_root / "leerie.toml").write_text(
        "model = haiku\nmodel_planner = haiku\n")
    monkeypatch.setenv("LEERIE_MODEL", "haiku")
    monkeypatch.setenv("LEERIE_MODEL_PLANNER", "haiku")
    models = leerie.resolve_models(repo_root,
...
def test_full_precedence_per_worker(leerie, repo_root, monkeypatch):
    ...
    assert leerie.resolve_models(repo_root, ns())["planner"] == "haiku"
    ...
    assert leerie.resolve_models(repo_root, ns(model="sonnet"))["planner"] == "sonnet"
...
def test_bad_per_worker_toml_dies(leerie, repo_root, capsys):
    (repo_root / "leerie.toml").write_text("model_classifier = nope\n")
    with pytest.raises(SystemExit) as exc:
        leerie.resolve_models(repo_root, ns())
    assert exc.value.code != 0
    err = capsys.readouterr().err
...
def test_empty_env_treated_as_unset(leerie, repo_root, monkeypatch):
    ...
    monkeypatch.setenv("LEERIE_MODEL", "")
    monkeypatch.setenv("LEERIE_MODEL_PLANNER", "   ")
    models = leerie.resolve_models(repo_root, ns())
...
def test_every_alias_accepted_in_global_env(leerie, repo_root, monkeypatch):
    for alias in leerie.MODEL_VALUES:
        monkeypatch.setenv("LEERIE_MODEL", alias)
        models = leerie.resolve_models(repo_root, ns())
        assert {w: models[w] for w in WORKERS} == {w: alias for w in WORKERS}


def test_worker_types_match_expected_set(leerie):
