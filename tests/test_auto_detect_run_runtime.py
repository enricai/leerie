...
`--runtime` enum validation and EC2 fail-closed behavior on each of the five
run-id-bearing verbs this subtask's scope note names: the detection helper
plus the `--runtime` enum validation it feeds, not any individual verb's EC2
*action* (that lands in feat-005..feat-008). None of these verbs need
`LEERIE_FLY_APP`/stubbed `flyctl`/`aws` to reach the EC2 fail-closed message,
since it fires before any remote dispatch.
"""
...
def test_stop_accepts_explicit_ec2_enum_but_fails_closed(tmp_path):
    """--runtime ec2 must clear the enum-validation gate (not rejected as
    unknown) but --stop's EC2 action isn't wired yet (feat-005), so it
    fails closed with an accurate message rather than silently mis-routing
    to the Fly or local path."""
    state_dir = tmp_path / "state"
    _make_e2e_run(state_dir, "r1")
...
def test_stop_autodetects_ec2_sidecar_and_fails_closed(tmp_path):
    state_dir = tmp_path / "state"
