...
phase-sequence body extracted from `_orchestrate()`) and `_settle_subtask()`
...
def test_orchestrate_calls_absorb_supplied_answers_on_resume():
    ...
    assert "if args.resume:" in body, (
        "_orchestrate() must keep its `if args.resume:` branch — "
    ...
    else_idx = body.index("\n    else:", resume_idx)
    absorb_idx = body.index("_absorb_supplied_answers(")
    assert resume_idx < absorb_idx < else_idx, (
        "_absorb_supplied_answers() must be called inside the "
...
def test_settle_subtask_has_needs_clarification_branch():
    """_settle_subtask() must route a worker's `needs-clarification`
    result through _surface_clarification + re-spawn. The unit tests
    for _validate_result and the schema cover the contract on the
    ...
    remove the branch and _validate_result would still reject malformed
    ...
    assert 'status == "needs-clarification"' in body, (
        "_settle_subtask() must have an `if status == "
        '"needs-clarification":` branch to route the new exit '
        "through _surface_clarification + re-spawn. Without it, the "
    ...
    # The branch must call _surface_clarification — that's how the
    ...
    assert "_surface_clarification(" in branch, (
        "the needs-clarification branch must call "
        "_surface_clarification() — that's how the question is "
...
def test_orchestrate_calls_phase_reconcile_between_plan_and_schedule():
    ...
    `phase_plan` and its result must feed `_schedule`. If a future
    refactor swapped the order (_schedule → reconcile) the scheduler
    ...
    schedule_idx = body.find("_schedule(plans)")
    assert plan_idx >= 0, "_orchestrate() must call phase_plan()"
    assert reconcile_idx >= 0, (
        "_orchestrate() must call phase_reconcile() between phase_plan "
        "and _schedule. Without it, cross-domain capability-tag "
        "mismatches that the planners produce slip through to "
        "_validate_plan and abort the run (DESIGN §5, §17).")
    assert schedule_idx >= 0, "_orchestrate() must call _schedule()"
    assert plan_idx < reconcile_idx < schedule_idx, (
        "ordering must be: phase_plan → phase_reconcile → _schedule. "
        f"got phase_plan@{plan_idx}, "
        f"phase_reconcile@{reconcile_idx}, _schedule@{schedule_idx}. "
...
def test_orchestrate_reconcile_feeds_schedule_via_plans_var():
    ...
    variable that _schedule reads. Otherwise the reconciler runs but its
    output is discarded — a silent regression where the _schedule still
    ...
    assert "plans = await phase_reconcile(plans," in body, (
        ...
        "mutated) plan list feeds _schedule(). A call that throws "
        "away the return value would leave _schedule operating on the "
        "raw planner output.")
...
def test_run_final_conformance_calls_resolve_blt():
    """_run_final_conformance must call resolve_blt(repo_root) for the same
    ...
    """
    body = _function_body("_run_final_conformance")
    assert "resolve_blt(repo_root)" in body, (
        "_run_final_conformance must call resolve_blt(repo_root) — "
        "not _infer_build_lint_test() directly — so that .leerie/config.toml "
    ...
    assert "_infer_build_lint_test(" not in body, (
        "_run_final_conformance must NOT call _infer_build_lint_test() "
        "directly. Use resolve_blt(repo_root) instead, which delegates to "
        "_infer_build_lint_test() for any axis not declared in config.")
