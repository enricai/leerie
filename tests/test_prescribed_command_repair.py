"""`repair_prescribed_commands` — the adherence gate repairs instead of re-driving.
...
def test_the_incident_shape_repairs_to_a_clean_floor(leerie):
    ...
    sid = leerie.repair_prescribed_commands(plans, _INCIDENT)
    assert sid
...
def test_repair_costs_no_workers(leerie):
    ...
    import inspect
    src = inspect.getsource(leerie.repair_prescribed_commands)
...
def test_already_clean_plan_is_untouched(leerie):
    ...
    assert leerie.repair_prescribed_commands(plans, _INCIDENT) is None
    assert len(plans[0]["subtasks"]) == n_before


def test_no_commands_declines(leerie):
    plans = _plans(2)
    assert leerie.repair_prescribed_commands(
...
def test_empty_plan_declines_rather_than_raising(leerie):
    assert leerie.repair_prescribed_commands([], _INCIDENT) is None
    assert leerie.repair_prescribed_commands(
...
def test_does_not_attach_to_an_arbitrary_existing_subtask(leerie):
    ...
    leerie.repair_prescribed_commands(plans, _INCIDENT)
...
def test_synthesised_subtask_carries_every_command(leerie):
    ...
    plans = _plans(2)
    sid = leerie.repair_prescribed_commands(plans, pres)
...
def test_repair_is_idempotent(leerie):
    ...
    plans = _plans(2)
    first = leerie.repair_prescribed_commands(plans, _INCIDENT)
...
def test_gate_repairs_before_evaluating_the_floor(leerie):
    ...
    assert "repair_prescribed_commands(" in src, "gate does not repair"
    assert src.index("repair_prescribed_commands(") < src.index(
