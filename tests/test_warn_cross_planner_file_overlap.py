...
historical leerie runs: 0 false positives in the n=3 runs available
...
def test_no_overlap_is_silent(leerie, monkeypatch):
...
def test_overlap_within_same_planner_does_not_warn(leerie, monkeypatch):
...
def test_warn_reports_count(leerie, monkeypatch):
    lines = _capture_logs(leerie, monkeypatch)
    plans = [
        {"domain": "feat", "subtasks": [
            {"id": "feat-001",
...
def test_multiple_subtasks_per_planner_listed(leerie, monkeypatch):
...
def test_plan_with_no_subtasks(leerie, monkeypatch):
...
def test_missing_files_likely_touched_is_safe(leerie, monkeypatch):
