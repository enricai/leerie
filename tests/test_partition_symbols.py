...
Covers the two deterministic tiers and the coverage backstop:
  - _partition_lines()            — tier 2: contiguous line-window tiling
  - _partition_symbols_by_line()  — tier 1: function-boundary tiling of [1, EOF]
...
def test_partition_lines_offset_range(leerie):
    ...
    w = leerie._partition_lines(5113, 6846, 700)
    assert _tiles_exactly(w, 5113, 6846)
    assert all((b - a + 1) <= 700 for a, b in w)


def test_partition_lines_single_window(leerie):
    assert leerie._partition_lines(1, 100, 500) == [(1, 100)]


def test_partition_lines_empty_range(leerie):
    assert leerie._partition_lines(10, 5, 200) == []


def test_partition_lines_degenerate_span(leerie):
    # max_span < 1 → whole range as one window (mirrors _partition_files guard).
    assert leerie._partition_lines(1, 100, 0) == [(1, 100)]


def test_partition_lines_span_one(leerie):
    w = leerie._partition_lines(1, 3, 1)
    assert w == [(1, 1), (2, 2), (3, 3)]
    assert _tiles_exactly(w, 1, 3)


# ---------------------------------------------------------------------------
# _partition_symbols_by_line — tier 1
# ---------------------------------------------------------------------------
...
def test_symbols_tile_whole_file(leerie):
    ...
    regions = leerie._partition_symbols_by_line(ranges, 300, 700)
...
def test_symbols_split_when_over_cap(leerie):
    ...
    ranges = [("a", 1, 100), ("b", 101, 200), ("c", 201, 300),
              ("d", 301, 400)]
    regions = leerie._partition_symbols_by_line(ranges, 400, 150)
...
def test_symbols_gap_attaches_to_preceding(leerie):
    ...
    regions = leerie._partition_symbols_by_line(ranges, 600, 700)
...
def test_symbols_giant_function_is_own_region(leerie):
    ...
    regions = leerie._partition_symbols_by_line(ranges, 2000, 700)
...
def test_symbols_no_ranges_returns_whole_file(leerie):
    ...
    assert leerie._partition_symbols_by_line([], 500, 700) == [(1, 500)]
...
def test_symbols_ignores_out_of_bounds_ranges(leerie):
    ...
    regions = leerie._partition_symbols_by_line(ranges, 500, 700)
    assert _tiles_exactly(regions, 1, 500)


def test_symbols_empty_file(leerie):
    assert leerie._partition_symbols_by_line([], 0, 700) == []
...
def test_compose_tier1_then_tier2_tiles_file(leerie):
    ...
    cap = 700
    tier1 = leerie._partition_symbols_by_line(ranges, total, cap)
    final = []
    for lo, hi in tier1:
        if (hi - lo + 1) > cap:
            final.extend(leerie._partition_lines(lo, hi, cap))
        else:
            final.append((lo, hi))
