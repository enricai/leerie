"""Tests for _build_repo_map() and _rank_repo_map() (DESIGN §5½ (P6)).

Covers:
- _build_repo_map on a small multi-file Python fixture produces defs→refs edges
...
- _rank_repo_map biases toward seed files/symbols and returns a subgraph whose
...
# parser _build_repo_map degrades to an empty graph and the assertions fail
...
# ---------------------------------------------------------------------------
# _build_repo_map — symbol extraction
# ---------------------------------------------------------------------------

class TestBuildRepoMapSymbols:
    """_build_repo_map extracts defs and builds refs edges."""

    def test_defs_extracted_per_file(self, leerie, tmp_path):
        repo = tmp_path / "repo"
        _write_fixture_repo(repo)
        repo_map = leerie._build_repo_map(repo, tmp_path / "leerie-root")
...
# ---------------------------------------------------------------------------
# _build_repo_map — mtime cache behaviour
# ---------------------------------------------------------------------------
...
class TestBuildRepoMapCache:
    ...
    def test_cache_dir_created_on_first_use(self, leerie, tmp_path):
        ...
        repo = tmp_path / "repo"
        _write_fixture_repo(repo)
        leerie._build_repo_map(repo, leerie_root)
    ...
    def test_unchanged_file_served_from_cache(self, leerie, tmp_path):
        ...
        repo_map2 = leerie._build_repo_map(repo, leerie_root)
...
# ---------------------------------------------------------------------------
# _rank_repo_map — PageRank + token budget
# ---------------------------------------------------------------------------

class TestRankRepoMap:
    """_rank_repo_map biases toward seeds and fits within the token budget."""
    ...
    def test_returns_string(self, leerie):
        ...
        result = leerie._rank_repo_map(repo_map, ["main.py"], [], 1000)
        assert isinstance(result, str)

    def test_fits_within_token_budget(self, leerie):
        repo_map = self._fixture_map()
        budget = 50  # tight budget
        result = leerie._rank_repo_map(repo_map, ["main.py"], [], budget)
    ...
    def test_seed_file_appears_in_result(self, leerie):
        repo_map = self._fixture_map()
        result = leerie._rank_repo_map(repo_map, ["utils.py"], [], 500)
    ...
    def test_seed_symbol_biases_result(self, leerie):
        ...
        result = leerie._rank_repo_map(repo_map, [], ["helper"], 500)
        assert "utils.py" in result
    ...
    def test_no_seed_returns_something_when_map_nonempty(self, leerie):
        repo_map = self._fixture_map()
        result = leerie._rank_repo_map(repo_map, [], [], 1000)
        assert result != ""

    def test_uses_default_cap_when_budget_none(self, leerie):
        repo_map = self._fixture_map()
        result_none = leerie._rank_repo_map(repo_map, ["utils.py"], [], None)
        result_cap = leerie._rank_repo_map(
            repo_map, ["utils.py"], [],
    ...
    def test_result_contains_symbol_names(self, leerie):
        repo_map = self._fixture_map()
        result = leerie._rank_repo_map(repo_map, ["utils.py"], [], 2000)
    ...
    def test_deterministic_same_inputs(self, leerie):
        repo_map = self._fixture_map()
        r1 = leerie._rank_repo_map(repo_map, ["main.py"], ["helper"], 500)
        r2 = leerie._rank_repo_map(repo_map, ["main.py"], ["helper"], 500)
        assert r1 == r2

    def test_very_tight_budget_returns_one_entry_or_empty(self, leerie):
        repo_map = self._fixture_map()
        # 5-token budget: fits zero or at most one short entry
        result = leerie._rank_repo_map(repo_map, ["utils.py"], [], 5)
        approx = max(1, len(result.encode()) // 4)
        assert approx <= 5
