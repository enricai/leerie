...
`prompts/patch_generator.md`). User reported it may not be working
properly; the concrete defect was that `skills/llm-self-heal/SKILL.md`
invoked an unregistered `Agent({subagent_type: "patch-generator"})`
subagent for patch generation instead of a `claude -p` worker,
breaking the heal loop and violating DESIGN §14's subprocess contract
and CLAUDE.md's "No subagent spawning" rule.

Fixed in #32: the patch-generation step now runs a direct `claude -p`
invocation mirroring the skill's own replay mechanics and the
orchestrator's `request_patch()`, with the anchor-must-be-a-literal-
substring validation applied mechanically before a patch is accepted.
Still marked `PARTIAL` — the code-level fix is landed but end-to-end
runtime verification of the heal loop is a separate, unaddressed
testing concern.
