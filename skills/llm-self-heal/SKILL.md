...
<objective>
Drive the autonomous heal loop for one or more leerie `call_type`s that
...
- `--heal-dir <dir>` (optional): where to write heal-loop state and
  ...
  `LEERIE_HEAL_DIR` / `leerie.toml heal_dir`.
...
- `--json-schema <schema>` (the same schema used for this call_type in
  the live orchestrator, per `SCHEMAS` in `orchestrator/leerie.py`)
