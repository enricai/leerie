...
Two traps recorded in `tests/test_resume_positional_run_id.py`: reverting
only the *wiring* (helper defined but uncalled) must fail — a
present-but-inert fix is the failure mode that let the coverage gate ship
— and the safety proof that `args.task` is read only on the non-resume
branch walks the AST of `_run_phases` (see `docs/TESTING.md` for the full
detail).
